...
def my_app(cfg: DictConfig) -> None:
    ...
    label_transform_with_resize = T.Compose(
        [T.Resize((small_imsize, small_imsize)), ToTargetTensor()]
    )
    ...
    class CodeSpaceTable(torch.nn.Module):
        def __init__(self, continuous, n_images, dim, h, w):
            ...
            self.code_space = torch.nn.Parameter(torch.randn(n_images, dim, h, w) * 0.1)
    ...
    for i in tqdm(range(cfg.epochs)):
        ...
        if continuous:
            ...
            ent_reg_term = (
                -cfg.global_ent_weight * ent_global - cfg.local_ent_weight * ent_local
            )
            ...
            with torch.no_grad():
                for idx, img_idx in enumerate(ind[:n_images]):
                    ...
                    if not continuous:
                        ax[1, idx].imshow(
                            mark_boundaries(
                                plot_img.cpu(), code.argmax(1)[idx].cpu().numpy()
                            )
                        )
                    else:
